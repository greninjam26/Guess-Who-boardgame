...
| 2026-09-15 05:10 UTC | `0a770473a8520e4df9e9d45bef3e35b0b2c9c348` | Replacement-host bootstrap gate: a temporary `t3.micro` using the live host's AMI, instance role and restricted security group downloaded the fixed eight-file candidate bundle from its immutable S3 prefix and passed its SHA-256 check. Starting from a clean Amazon Linux host, bootstrap ran twice successfully; rendered runtime-file hashes stayed identical; PostgreSQL, Caddy and CloudWatch Agent were active; the database and backup timer existed; environment files were mode `640`; and PostgreSQL listened only on loopback with nothing on port 8080. SSM command `7c09a164-e0e2-4c8a-bfee-e6c6e35d81ce` exited 0. The temporary instance and its delete-on-termination volume were removed, then the production smoke test passed 7 of 7 | Codex |
