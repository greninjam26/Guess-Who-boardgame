...
- AWS for the public server: one EC2 instance behind Caddy — see
...
installers on the releases page were built before it existed and still connect
to `localhost`, so until installers built against it are released, play on it
from a source build:
...
- Background music is generated by `tools/BackgroundTrack.java` rather than
  ...
  deliberately plain.
- The released installers connect to `localhost`. The public server went up
  after they were built; see [Play Online](#play-online) for using it until
  installers built against it are released.
